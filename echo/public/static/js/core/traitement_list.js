function supprimer(pk) {
    swal.fire({
        ...
        if (result.value) {
            window.location.replace("/traitements/" + pk + "/supprimer");
        }
    });
}
...
$(document).ready(() => {
    const initTable = function () {
        ...
        const table = $('#datatable_traitements').DataTable({
            language: {
            ...
            processing: true,
            serverSide: true,
            ajax: { url: "/traitements/recherche/", type: 'POST' },
            ...
            columnDefs: [
                {
                    targets: 2, title: 'Actions', orderable: false, width: '100px',
