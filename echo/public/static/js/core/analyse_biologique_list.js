function supprimer(pk) {
    swal.fire({
        ...
        if (result.value) {
            window.location.replace("/analyses-biologiques/" + pk + "/supprimer");
        }
    });
}
...
$(document).ready(() => {
    const initTable = function () {
        ...
        const table = $('#datatable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: { url: "/analyses-biologiques/recherche/", type: 'POST' },
            ...
            columnDefs: [
                {
                    targets: 3, title: 'Actions', orderable: false, width: '100px',
