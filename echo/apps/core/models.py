...
class Compte(models.Model):
    ...
    adresse = models.ForeignKey(
        Adresse, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    distribution = models.CharField(
        max_length=128, choices=DISTRIBUTIONS, default=GYNECOLOGIE
    )
...
def repertoire_utilisateur(instance, filename):
    return "comptes/compte_{0}/utilisateurs/{1}/{2}".format(
        instance.compte.pk, instance.user.pk, filename
    )
...
class Profil(CompteModelBase):
    ...
    signature = models.ImageField(
        upload_to=repertoire_utilisateur,
    ...
    default_device = models.ForeignKey(
        "Device", on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        self.signature = InMemoryUploadedFile(
            output, "ImageField", filename, mime, sys.getsizeof(output), None
        )
        # super().save(*args, **kwargs)
...
class ParametresCompte(models.Model):
    ...
    praticien_defaut = models.ForeignKey(
        Medecin, on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    logo = models.ImageField(
        upload_to=logo_storage_path, max_length=128, blank=True, null=True
    )
    logo_a4 = models.ImageField(
        upload_to=logo_storage_path_a4, max_length=128, blank=True, null=True
    )
    footer = models.ImageField(
        upload_to=footer_storage_path, max_length=128, blank=True, null=True
    )
    footer_a4 = models.ImageField(
        upload_to=footer_storage_path_a4, max_length=128, blank=True, null=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        self.logo = InMemoryUploadedFile(
            output, "ImageField", "logo.png", "image/png", sys.getsizeof(output), None
        )
        super().save(*args, **kwargs)
...
class InformationsConjoint(models.Model):
    ...
    consanguinite_conjoint = models.CharField(
        max_length=256,
        default="",
        blank=True,
        null=True,
...
class Patient(CompteModelBase, InformationsConjoint, InformationsMutuelle):
    ...
    sexe = models.CharField(
        max_length=1, default="F", choices=[("F", _("Femme")), ("H", _("Homme"))]
    )
    ...
    origine = models.CharField(
        ...
        null=True,
        choices=[
            ("1", _("Europe/Afrique du nord")),
            ("2", _("Afrique sud-saharienne")),
            ("3", _("Amérique latine")),
            ("4", _("Asie")),
            ("10", _("Autre")),
        ],
    ...
    def antecedents_obstetriques(self):
        return AntecedentObstetrique.objects.filter(
            patient=self, sous_categorie__categorie=4
    ...
    def fichiers(self):
        ...
        for result in self.fichierpatient_set.values(
            "dossier__nom", "nom", "fichier", "pk", "date"
        ).order_by("dossier__nom", "nom"):
            fichiers[result["dossier__nom"]].append(
                {
                    "pk": result["pk"],
                    "nom": result["nom"],
                    "chemin": result["fichier"],
                    "date": result["date"],
                }
            )
    ...
    def check_doublon_consultation(self, motif, date):
        ...
        return self.consultation_set.filter(
            motif=motif,
            date__day=date.day,
    ...
    def antecedents_medico_chirurgicaux_compresse(self):
        if not self.antecedents_medico_chirurgicaux:
            return ""
        res = (
            self.antecedents_medico_chirurgicaux.replace("<p>", "")
            .replace("</p>", ", ")
            .replace("<br />", ", ")
        )
    ...
    def antecedents_gynecologiques_compresse(self):
        if not self.antecedents_gynecologiques:
            return ""
        res = (
            self.antecedents_gynecologiques.replace("<p>", "")
            .replace("</p>", ", ")
            .replace("<br />", ", ")
        )
        if len(res) > 2 and res[-2] == ",":
    ...
    def mesures_jour(self):
        ...
        mesures = self.mesurespatient_set.filter(
            date__day=t.day, date__month=t.month, date__year=t.year
        ).first()
...
def repertoire_patient(instance, filename):
    return "comptes/compte_{0}/patients/{1}/{2}".format(
        instance.patient.compte.pk, instance.patient.pk, filename
    )
...
class CatgeorieAntecedent(models.Model):
    libelle = models.CharField(max_length=128)
    libelle_en = models.CharField(max_length=128, blank=True, default="")
    libelle_ar = models.CharField(max_length=128, blank=True, default="")
    libelle_es = models.CharField(max_length=128, blank=True, default="")
...
class Antecedent(CompteModelBase):
    sous_categorie = models.ForeignKey(
        SousCatgeorieAntecedent, on_delete=models.CASCADE
    )
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE)
...
class Rdv(CompteModelBase):
    ...
    )  # Pour retrouver les rdv modifiés
    prenom = models.CharField(max_length=128)
    nom = models.CharField(max_length=128, blank=True, null=True)
    nom_naissance = models.CharField(max_length=128, blank=True, null=True)
    telephone = models.CharField(max_length=20, blank=True, null=True)
    ...
    )  # Motif de modification/annulation
    ...
    def is_missed(self):
        ...
        if p:
            return (
                p.consultation_set.filter(
                    date__day=self.debut.day,
                    date__month=self.debut.month,
                    date__year=self.debut.year,
                ).count()
                == 0
            )
        return False
...
class TypeOrdonnance(CompteModelBase):
    ...
    categorie = models.CharField(
        max_length=32, choices=CATEGORIE_CHOICES, default=CATEGORIE_AUTRE
    )
...
class Ordonnance(models.Model):
    ...
    type = models.ForeignKey(
        TypeOrdonnance, on_delete=models.SET_NULL, blank=True, null=True
    )
    text = models.TextField()
    praticien = models.ForeignKey(
        Medecin, on_delete=models.SET_NULL, blank=True, null=True
...
class Certificat(models.Model):
    ...
    type = models.CharField(
        max_length=128, default=CERTIFICAT_MEDICAL, choices=TYPES_CERTIFICATS
    )
    text = models.TextField()
    praticien = models.ForeignKey(
        Medecin, on_delete=models.SET_NULL, blank=True, null=True
...
class AbsenceMedecin(CompteModelBase):
    ...
    motif = models.ForeignKey(
        MotifAbsence, on_delete=models.SET_NULL, blank=True, null=True
    )
    praticien = models.ForeignKey(
        Medecin,
    ...
    praticien_remplacant = models.ForeignKey(
        Medecin,
        on_delete=models.SET_NULL,
        blank=True,
        null=True,
...
class ProgrammeOperatoire(CompteModelBase):
    ...
    lieu_accouchement = models.ForeignKey(
        Etablissement, blank=True, null=True, on_delete=models.SET_NULL
    )
    observation = models.CharField(max_length=256, blank=True, null=True)
    praticien = models.ForeignKey(
        Medecin, on_delete=models.SET_NULL, blank=True, null=True
...
class ListeChoix(models.Model):
    ...
    actif = models.BooleanField(default=True)
    normale = models.BooleanField(
        default=False
    )  # Utilisé avec la fonction "Tout normal" des formulaires
...
class Grossesse(BaseModel):
    ...
    type_grossesse = models.CharField(
        max_length=256,
        default="",
        blank=True,
        null=True,
        choices=[
            ("", ""),
            ("bichoriale_biamniotique", "bichoriale biamniotique"),
            ("monochoriale_biamniotique", "monochoriale biamniotique"),
            ("monochoriale_monoamniotique", "monochoriale monoamniotique"),
        ],
    )
    type_grossesse_v2 = models.ForeignKey(
        ListeChoix,
        related_name="type_grossesse",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    conception = models.CharField(
        ...
        choices=[
            ("", ""),
            ("spontannee", "Spontannée"),
            ("iac", "IAC"),
            ("fiv", "FIV"),
        ],
    )
    conception_v2 = models.ForeignKey(
        ListeChoix,
        related_name="conception",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    score_herman_1 = models.CharField(
        max_length=256,
        default="",
        null=True,
        blank=True,
        choices=[
            ("", ""),
            ("cn_non_connue", "CN non connue"),
            ("cn_non_mesuree", "CN non mesurée"),
            ("cn_non_significative", "CN non significative pour cette LCC"),
            ("score_9", "Score de Herman 9"),
            ("score_4", "Score de Herman < 4"),
        ],
    )
    score_herman_1_v2 = models.ForeignKey(
        ListeChoix,
        related_name="score_herman_1",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    score_herman_2 = models.CharField(
        max_length=256,
        default="",
        null=True,
        blank=True,
    ...
    score_herman_2_v2 = models.ForeignKey(
        ListeChoix,
        related_name="score_herman_2",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    score_herman_3 = models.CharField(
        max_length=256,
        default="",
        null=True,
        blank=True,
    ...
    score_herman_3_v2 = models.ForeignKey(
        ListeChoix,
        related_name="score_herman_3",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    msres_1_type_v2 = models.ForeignKey(
        ListeChoix,
        related_name="msres_1_type",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    msres_2_type_v2 = models.ForeignKey(
        ListeChoix,
        related_name="msres_2_type",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    screening_premier_trimestre = models.CharField(
        max_length=128, blank=True, null=True
    )
    screening_deuxieme_trimestre = models.CharField(
        max_length=128, blank=True, null=True
    )
    caryotype_type = models.ForeignKey(
        ListeChoix,
        related_name="caryotype",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    diabete_v2 = models.ForeignKey(
        ListeChoix,
        related_name="diabete",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    toxoplasmose_v2 = models.CharField(
        max_length=256,
        default="",
        blank=True,
        null=True,
        choices=[("", "Non défini"), ("0", "Non immunisée"), ("1", "Immunisée")],
    )
    ...
    lieu_accouchement_principal = models.CharField(
        max_length=1,
        blank=True,
    ...
    def terme_disp(self):
        ...
        if ddr is not None:
            t = (
                datetime.date.today() - ddr
            )  # pour ajouter + datetime.timedelta(duree_gros)
...
class AntecedentObstetrique(Antecedent):
    grossesse_patient = models.ForeignKey(
        Grossesse, on_delete=models.CASCADE, blank=True, null=True
    )
    grossesse = models.ManyToManyField(ListeChoix, related_name="grossesse", blank=True)
    issue_grossesse = models.ForeignKey(
        ListeChoix,
        related_name="issue_grossesse",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    mise_en_travail = models.ForeignKey(
        ListeChoix,
        related_name="mise_en_travail",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    analgesie = models.ForeignKey(
        ListeChoix,
        related_name="analgesie",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    etat_sante_2 = models.ForeignKey(
        ListeChoix,
        related_name="etat_sante_2",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    etat_sante_3 = models.ForeignKey(
        ListeChoix,
        related_name="etat_sante_3",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    perinee = models.ForeignKey(
        ListeChoix,
        related_name="perinee",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    suite_couche_type = models.ForeignKey(
        ListeChoix,
        related_name="suite_couche_type",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    suite_couche_detail = models.ForeignKey(
        ListeChoix,
        related_name="suite_couche_detail",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    atcd_surligner = models.TextField(blank=True, null=True)
    evacuation_grossesse = models.ForeignKey(
        ListeChoix,
        related_name="evacuation_grossesse",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
...
class ConsultationEchoPelvienneBase(models.Model):
    titre_echo_pelvienne = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_titre_echo_pelvienne",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    # Uterus
    position_uterus = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_position_uterus",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    lateralisation = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_lateralisation",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    volume_uterin_commentaire = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_volume_uterin_commentaire",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    asymetrie = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_asymetrie",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    mobilite = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_mobilite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    structures = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_structures",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    commentaires_myometre = models.TextField(blank=True, null=True)
    cavite = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_cavite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    malformation = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_malformation",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    largeur_interostiale = models.FloatField(blank=True, null=True)
    ligne_cavitaire = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ligne_cavitaire",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    hysterometrie = models.FloatField(blank=True, null=True)
    adenomyose = models.ManyToManyField(
        ListeChoix, related_name="%(app_label)s_%(class)s_adenomyose", blank=True
    )
    adenomyose_conclusion = models.ManyToManyField(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_adenomyose_conclusion",
        blank=True,
    )
    #######
    type_dispositif_intra_uterin = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_type_dispositif_intra_uterin",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    localisation_dispositif_intra_uterin = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_localisation_dispositif_intra_uterin",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    anomalies_dispositif_intra_uterin = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_anomalies_dispositif_intra_uterin",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    dispositif_intra_tubaire = models.BooleanField(blank=True, null=True)
    endometre_visualisation = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_endometre_visualisation",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    endometre_echogenicite = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_endometre_echogenicite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    col_aspect = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_col_aspect",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    col_vascularisation = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_col_vascularisation",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    # Ovaire gauche
    ovaire_gauche_visibilite = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_gauche_visibilite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ovaire_gauche_aspect = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_gauche_aspect",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    ovaire_gauche_mobilite = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_gauche_mobilite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ovaire_gauche_accessibilite = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_gauche_accessibilite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ovaire_gauche_follicules = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_gauche_follicules",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    # Ovaire gauche
    ovaire_droit_visibilite = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_droit_visibilite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ovaire_droit_aspect = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_droit_aspect",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    ovaire_droit_mobilite = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_droit_mobilite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ovaire_droit_accessibilite = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_droit_accessibilite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ovaire_droit_follicules = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_ovaire_droit_follicules",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    cul_de_sac_latero = models.ForeignKey(
        ListeChoix,
        related_name="%(app_label)s_%(class)s_cul_de_sac_latero",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
...
class ConsultationGynecologique(
    Consultation, InformationsConjoint, ConsultationEchoPelvienneBase
):
    motif_consultation = models.ForeignKey(
        ListeChoix,
        related_name="motif_consultation",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    abondance = models.ForeignKey(
        ListeChoix,
        related_name="abondance",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    douleur = models.ForeignKey(
        ListeChoix,
        related_name="douleur",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    syndrome_premenstruel = models.ForeignKey(
        ListeChoix,
        related_name="syndrome_premenstruel",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    # Rapports sexuels
    presence_rapports_sexuels = models.ForeignKey(
        ListeChoix,
        related_name="presence_rapports_sexuels",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    # Contraception
    mode_contraception = models.ForeignKey(
        ListeChoix,
        related_name="mode_contraception",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    observance = models.ForeignKey(
        ListeChoix,
        related_name="observance",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    satisfaction = models.ForeignKey(
        ListeChoix,
    ...
    examen_sous_speculum = models.ForeignKey(
        ListeChoix,
        related_name="examen_sous_speculum",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    # Autres
    effets_indesirables = models.ForeignKey(
        ListeChoix,
    ...
    prochaine_consultation_approx = models.CharField(
        max_length=128, blank=True, null=True
    )
...
class ConsultationColposcopie(Consultation):
    indications = models.ForeignKey(
        ListeChoix,
        related_name="indications_colposcopie",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    test_hpv = models.ForeignKey(
        ListeChoix,
        related_name="test_hpv",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    examen_sans_preparation = models.ForeignKey(
        ListeChoix,
        related_name="examen_sans_preparation",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    commentaires_2 = models.TextField(blank=True, null=True)
    acide_acetique = models.ForeignKey(
        ListeChoix,
        related_name="acide_acetique",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    tag = models.ForeignKey(
        ListeChoix, related_name="tag", blank=True, null=True, on_delete=models.SET_NULL
    )
...
class Myome(models.Model):
    ...
    type_figo = models.ForeignKey(
        ListeChoix,
        related_name="type_figo",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    situation_coupe_longitudinale = models.ForeignKey(
        ListeChoix,
        related_name="situation_coupe_longitudinale",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    situation_coupe_transversale = models.ForeignKey(
        ListeChoix,
        related_name="situation_coupe_transversale",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    contours = models.ForeignKey(
        ListeChoix,
        related_name="countours",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    calcifications = models.ForeignKey(
        ListeChoix,
        related_name="calcifications",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    vascularisation = models.ForeignKey(
        ListeChoix,
        related_name="vascularisation",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
...
class ConsultationObstetrique(Consultation):
    ...
    notch_droit = models.ForeignKey(
        ListeChoix,
        related_name="notch_droit",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    notch_gauche = models.ForeignKey(
        ListeChoix,
        related_name="notch_gauche",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    col_long = models.FloatField(blank=True, null=True)
    col_entonnoir = models.ForeignKey(
        ListeChoix,
        related_name="col_entonnoir",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    examen_sous_speculum = models.ForeignKey(
        ListeChoix,
        related_name="obs_examen_sous_speculum",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    pelvis_maternel = models.ForeignKey(
        ListeChoix,
        related_name="pelvis_maternel",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    lmc = models.ForeignKey(
        ListeChoix, related_name="lmc", blank=True, null=True, on_delete=models.SET_NULL
    )
...
class DonneesFoetus(models.Model):
    ...
    activite_cardiaque = models.ForeignKey(
        ListeChoix,
        related_name="activite_cardiaque_foetus",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    opn_presence = models.CharField(
        max_length=1, blank=True, null=True, choices=[("0", "Absent"), ("1", "Présent")]
    )
    ...
    doppler_cordon_diastole = models.ForeignKey(
        ListeChoix,
        related_name="doppler_cordon_diastole",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    doppler_dv_onde = models.ForeignKey(
        ListeChoix,
        related_name="doppler_dv_onde",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_crane = models.ForeignKey(
        ListeChoix,
        related_name="morpho_crane",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_struct = models.ForeignKey(
        ListeChoix,
        related_name="morpho_struct",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_face = models.ForeignKey(
        ListeChoix,
        related_name="morpho_face",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_thorax = models.ForeignKey(
        ListeChoix,
        related_name="morpho_thorax",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_coeur = models.ForeignKey(
        ListeChoix,
        related_name="morpho_coeur",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_pole_cepha = models.ForeignKey(
        ListeChoix,
        related_name="morpho_pole_cepha",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_abdo = models.ForeignKey(
        ListeChoix,
        related_name="morpho_abdo",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_digest = models.ForeignKey(
        ListeChoix,
        related_name="morpho_digest",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_urine = models.ForeignKey(
        ListeChoix,
        related_name="morpho_urine",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_rachis = models.ForeignKey(
        ListeChoix,
        related_name="morpho_rachis",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_membres = models.ForeignKey(
        ListeChoix,
        related_name="morpho_membre",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_oge = models.ForeignKey(
        ListeChoix,
        related_name="morpho_oge",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    annexe_liquide_amnio = models.ForeignKey(
        ListeChoix,
        related_name="annexe_liquide_amnio",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_liquide_amnio = models.ForeignKey(
        ListeChoix,
        related_name="morpho_liquide_amnio",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_trophoblaste_localisation = models.ForeignKey(
        ListeChoix,
        related_name="morpho_trophoblaste_localisation",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_trophoblaste_aspect = models.ForeignKey(
        ListeChoix,
        related_name="morpho_trophoblaste_aspect",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_decol = models.ForeignKey(
        ListeChoix,
        related_name="morpho_decol",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_placenta = models.ForeignKey(
        ListeChoix,
        related_name="morpho_placenta",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_cordon = models.ForeignKey(
        ListeChoix,
        related_name="morpho_cordon",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    morpho_cou = models.ForeignKey(
        ListeChoix,
        related_name="morpho_cou",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
...
class ConsultationEcho11SA(ConsultationObstetrique):
    sac_gestationnel_localisation = models.ForeignKey(
        ListeChoix,
        related_name="sac_gestationnel_localisation",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    sac_gestationnel_tonicite = models.ForeignKey(
        ListeChoix,
        related_name="sac_gestationnel_tonicite",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    sac_gestationnel_trophoblaste = models.ForeignKey(
        ListeChoix,
        related_name="sac_gestationnel_trophoblaste",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    sac_gestationnel_decollement = models.ForeignKey(
        ListeChoix,
        related_name="sac_gestationnel_decollement",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    embryon_visible = models.BooleanField(default=True)
    morpho_extremite_cephalique = models.ForeignKey(
        ListeChoix,
        related_name="morpho_extremite_cephalique",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
    ...
    activite_cardiaque = models.ForeignKey(
        ListeChoix,
        related_name="activite_cardiaque",
        blank=True,
        null=True,
        on_delete=models.SET_NULL,
    )
...
class TentativesHistoriquesPMA(BaseModel):
    ...
    interrogatoire = models.ForeignKey(
        "InterrogatoirePMA", related_name="tentatives", on_delete=models.CASCADE
    )
    date = models.DateField(blank=True, null=True)
    acte = models.CharField(max_length=32, blank=True, null=True, choices=TYPE_ACTE)
    resultat = models.CharField(
        max_length=32, blank=True, null=True, choices=RESULT_ACTE
    )
...
class BilanEndocrinienPMAFeminin(BilanEndocrinienPMA):
    interrogatoire = models.ForeignKey(
        "InterrogatoirePMA",
        related_name="bilans_endocriniens_feminin",
...
class BilanEndocrinienPMAMasculin(BilanEndocrinienPMA):
    interrogatoire = models.ForeignKey(
        "InterrogatoirePMA",
        related_name="bilans_endocriniens_masculin",
        on_delete=models.CASCADE,
    )
...
class SpermogrammePMA(BaseModel):
    interrogatoire = models.ForeignKey(
        "InterrogatoirePMA", related_name="spermogrammes", on_delete=models.CASCADE
    )
    date = models.DateField(blank=True, null=True)
...
class InterrogatoirePMA(InformationsConjoint):
    ...
    type_infertilite = models.CharField(
        max_length=32, default="Primaire", choices=TYPE_INFERTILITE
    )
    ...
    sperme_oat_gravite = models.CharField(
        max_length=32, blank=True, null=True, choices=SPERME_OAT_GRAVITE
    )
    ...
    sperme_azoo_gravite = models.CharField(
        max_length=32, blank=True, null=True, choices=SPERME_AZOO_GRAVITE
    )
    sperme_congenalation = models.BooleanField(default=False)
    sperme_congenalation_clinique = models.ForeignKey(
        Etablissement, on_delete=models.SET_NULL, blank=True, null=True
    )
    nb_paillettes = models.SmallIntegerField(blank=True, null=True)
    RENDEMENT = [("bon", "Bon"), ("mauvais", "Mauvais")]
    rendement = models.CharField(
        max_length=32, blank=True, null=True, choices=RENDEMENT
    )
...
class SuiviTraitementPMA(BaseModel):
    ...
    traitments_valeurs = models.ManyToManyField(
        TraitementPMA, through="TraitementValeurPMA", blank=True
    )
...
class TraitementValeurPMA(BaseModel):
    ...
    traitement = models.ForeignKey(
        TraitementPMA, on_delete=models.SET_NULL, blank=True, null=True
    )
...
class Reglement(models.Model):
    admission = models.ForeignKey(
        Admission, related_name="reglements", on_delete=models.CASCADE
    )
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE)
    praticien = models.ForeignKey(
        Medecin, on_delete=models.SET_NULL, blank=True, null=True
    ...
    cloture = models.ForeignKey(
        Cloture, on_delete=models.SET_NULL, blank=True, null=True
    )
    bordereau = models.ForeignKey(
        Bordereau, on_delete=models.SET_NULL, blank=True, null=True
    )
    date_creation = models.DateTimeField(default=django.utils.timezone.now)
...
class LigneReglement(models.Model):
    ...
    prix_initial = models.FloatField(
        blank=True, null=True
    )  # Montant total = montant prise en charge + ticket modérateur
    reglement = models.ForeignKey(
        Reglement,
        related_name="lignes_reglement",
        on_delete=models.CASCADE,
        blank=True,
        null=True,
    )
...
class Facture(models.Model):
    reglement = models.ForeignKey(
        Reglement, related_name="factures", on_delete=models.CASCADE
    )

    def __str__(self):
        return "{}, {}".format(
            self.reglement.patient.prenom, self.reglement.patient.nom
        )


def repertoire_images_utilisateur(compte_pk, patient_pk, nom_fichier):
    return "comptes/compte_{0}/patients/{1}/images/{2}".format(
        compte_pk, patient_pk, nom_fichier
    )
...
def upload_repertoire_images_utilisateur(instance, filename):
    ...
    path = repertoire_images_utilisateur(
        instance.consultation.patient.compte.pk,
...
class ImageConsultation(models.Model):
    ...
    image = models.ImageField(
        upload_to=upload_repertoire_images_utilisateur, blank=True, null=True
    )
    type = models.CharField(
...
class MesuresConsultation(models.Model):
    ...
    )  # JSON — same schema as SRConsultation.data
...
class AnalyseBiologique(CompteModelBase):
    ...
    type = models.CharField(
        max_length=32, choices=[("text", "Texte"), ("number", "Numérique")]
    )
...
class AnalyseOrderedManyToManyField(models.ManyToManyField):
    ...
    def value_from_object(self, object):
        ...
        print("IDS", aids)
        analyses = dict(
            (a.pk, a) for a in AnalyseBiologique.objects.filter(pk__in=aids)
        )
...
class CollectionAnalyseBiologique(CompteModelBase):
    ...
    def analyses_list(self):
        analyses = [
            a.analysebiologique
            for a in self.analyses.through.objects.filter(
                collectionanalysebiologique_id=self.id
            )
...
class PrescriptionAnalyseBiologique(models.Model):
    ...
    analyses = models.ManyToManyField(
        AnalyseBiologique, through="ResultatAnalyseBiologique"
    )
...
class ResultatAnalyseBiologique(models.Model):
    ...
    prescription = models.ForeignKey(
        PrescriptionAnalyseBiologique, on_delete=models.CASCADE
    )
...
class WorklistItem(models.Model):
    ...
    MPPS_STATUSES = [
        (MPPS_STATUS_PENDING, "pending"),
        (MPPS_STATUS_INPROGRESS, "inprogress"),
        (MPPS_STATUS_COMPLETED, "completed"),
        (MPPS_STATUS_DISCONTINUED, "discontinued"),
    ]
    consultation = models.ForeignKey(Consultation, on_delete=models.CASCADE)
...
class TemplateEdition(CompteModelBase):
    compte = models.ForeignKey(Compte, on_delete=models.CASCADE)
    categorie_consultation = models.ForeignKey(
        CategorieConsultation,
        related_name="categorie_consultation",
        on_delete=models.CASCADE,
    )
    motif_consultation = models.ForeignKey(
        MotifConsultation,
...
class SuperAdminProfile(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="super_admin_profile"
    )
