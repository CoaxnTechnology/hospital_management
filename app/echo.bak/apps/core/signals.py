...
DEFAULT_DISTRIBUTION_CATEGORIES = {
    "gyneco": [1, 2, 4, 7],
    "cardio": [5, 6, 4, 7],
    "general": [4, 7],
}
...
def creer_parametres_compte(sender, instance, **kwargs):
    ...
    # Keep default consultation visibility consistent for newly created accounts.
    if kwargs.get("created") and instance.categories_consultations.count() == 0:
        cat_ids = DEFAULT_DISTRIBUTION_CATEGORIES.get(
            instance.distribution, DEFAULT_DISTRIBUTION_CATEGORIES["gyneco"]
        )
        categories = CategorieConsultation.objects.filter(pk__in=cat_ids)
        instance.categories_consultations.set(categories)
